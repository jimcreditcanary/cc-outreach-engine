...
interface Props {
  ...
  searchParams: Promise<{ booked?: string; join?: string; error?: string }>;
}
...
export default async function PublicBookingPage({ params, searchParams }: Props) {
  ...
  if (sp.booked) {
    return (
      <Shell operatorName={page.operator_name}>
  ...
  return (
    <Shell operatorName={page.operator_name}>
      ...
      {loadFailed ? (
        ...
        <BookingForm slug={slug} slots={slots} durationMins={page.config.durationMins} />
      )}
    </Shell>
  );
}

function Shell({ operatorName, children }: { operatorName: string; children: React.ReactNode }) {
  return (
